...
│                                                                  │
│   ┌─────────────────┐        ┌──────────────────────────────┐   │
│   │  validateInput  │        │  Groq LLaMA 3.3-70b          │   │
│   │                 │───────▶│  "Is this tech-related?"      │   │
│   │  Topic: string  │        │  Returns: { isTechRelated }   │   │
│   └─────────────────┘        └──────────────────────────────┘   │
│                                        │                         │
│                              ┌─────────▼──────────┐             │
│                              │  ✅ true → proceed  │             │
...
│                                                                  │
│   messages = [{ role: "user", parts: [{ text: topic }] }]        │
│                                                                  │
│   ┌──────────────────────────────────────────────────────────┐   │
│   │                                                          │   │
│   │   ┌─────────────┐    tool call?    ┌─────────────────┐  │   │
│   │   │             │ ───────────────▶ │   TOOL ROUTER   │  │   │
│   │   │  Gemini     │                  │                 │  │   │
│   │   │  2.5-flash  │                  │  name="WebSearch│  │   │
│   │   │             │                  │  → WebSearch()  │  │   │
│   │   │  (reads     │                  │                 │  │   │
│   │   │  messages   │                  │  name="fetchUrl"│  │   │
│   │   │  array)     │                  │  → fetchUrl()   │  │   │
│   │   │             │ ◀─────────────── │                 │  │   │
│   │   └─────────────┘   push result    └────────┬────────┘  │   │
│   │          │          to messages             │            │   │
...
│   │   ┌──────▼──────────────────────────────────────────────┐  │   │
│   │   │           CONTEXT COMPRESSION                        │  │   │
│   │   │   if messages.length > 6 && length % 4 === 0        │  │   │
│   │   │                                                       │  │   │
│   │   │   keep originalQuery[0]                              │  │   │
│   │   │   summarize rest → Groq LLaMA (300 words)            │  │   │
│   │   │   messages = [originalQuery, summary]                │  │   │
│   │   │                                                       │  │   │
│   │   │   Token savings: ~70% reduction per compression      │  │   │
│   │   └───────────────────────────────────────────────────── ┘  │   │
│   │                                                          │   │
│   └──────────────────────────────────────────────────────────┘   │
│                                                                  │
...
│                                                                  │
│   {                                                              │
│     title, generatedAt, topic,                                   │
│     summary,                                                     │
│     sections: [{ heading, content, keyPoints, sources }],        │
│     techStack, keyFacts, useCases,                               │
│     limitations, futureOutlook, allSources                       │
│   }                                                              │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
